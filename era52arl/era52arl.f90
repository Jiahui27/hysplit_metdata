...
!==>compute the record offset based upon variable match
   ...
   NVAR = GV(KG)%NVAR(LL)
   DO K = 1, NVAR
      IF (KVAR(2:4) .EQ. GV(KG)%VARB(K, LL)) NV = K
      ! WRITE (*, *) 'Argument variable here: ', GV(KG)%VARB(K, LL),'   ',KVAR,len(GV(KG)%VARB(K, LL))
   END DO

   IF (NV .EQ. 0) THEN
      WRITE (*, *) 'ERROR pakrec: Variable not in CFG file'
      WRITE (*, *) 'Argument list variable: ', KVAR, ',', GV(KG)%VARB(1, LL), LL
      STOP
   END IF
